...
# vvvvvvvvvvvvvvvv 修改/新增 vvvvvvvvvvvvvvvv
from langchain.text_splitter import SpacyTextSplitter  # <-- 新增：导入Spacy分块器
# (移除了 numpy, scipy, ZhipuAIEmbeddings, langchain_core.embeddings.Embeddings 等不再需要的库)
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
def create_rule_generation_prompt() -> ChatPromptTemplate:
    ...
    system_message = SystemMessagePromptTemplate.from_template(
        "你是一位精通材料科学与工程的分析专家。你的任务是从给定的文本片段中，识别出所有实体（特别是材料、性质、性能），并总结出它们的各种不同称呼、别名或头衔。"
    )
...
# --- 移除：不再需要初始化Embedding模型 ---

# --- 4. 新增：预定义的Schema ---
# 这里可以根据需要预先定义好通用的实体和关系类型
PREDEFINED_ENTITY_TYPES = ["材料", "性质", "性能"]
PREDEFINED_RELATION_TYPES = ["属于", "表现", "应用"]


# --- 修改：重写文件读取函数以支持多种格式 ---
def read_supported_files(directory: str) -> List[Tuple[str, str]]:
    """
    读取目录下所有支持的文件（.txt, .pdf, .docx）并返回其内容。
    注意：此函数会跳过不支持的文件类型。
    """
    results = []
    supported_extensions = [".txt", ".pdf", ".docx"]
    logging.info(f"开始从目录 {directory} 读取支持的文件: {', '.join(supported_extensions)}")
    ...
    for filename in os.listdir(directory):
        ...
        try:
            ...
            if content.strip():
                ...
                logging.info(f"成功读取并提取文件内容: {filename}")
            else:
                logging.warning(f"文件 {filename} 为空或未能提取到文本内容。")
...
# --- 移除：SemanticChunker 类和 chunk_text 函数都已被移除 ---
...
def create_knowledge_extraction_prompt(entity_types: List[str], relation_types: List[str]) -> ChatPromptTemplate:
    system_message = SystemMessagePromptTemplate.from_template(
        ...
        "你必须严格遵守指定的实体和关系类型。"
        "至关重要的一点是：如果文本中出现同一个实体的不同名称或别名（例如 '氧化亚锡', 'SnO'），"
        "你必须将它们统一识别为最常见或最完整的规范名称（例如 'SnO'）。"
...
# --- 5. 新增：Schema来源选择函数 ---
def choose_schema_source() -> int:
    """
    提供用户界面，让用户选择Schema的来源。
    返回:
        1: 使用预定义类型
        2: 使用大模型动态生成
    """
    os.system('cls' if os.name == 'nt' else 'clear')
    while True:
        print("--- 请选择Schema（实体与关系类型）的定义方式 ---")
        print("\n1. 使用预定义类型")
        print("   - 类型包括：材料, 性质, 性能等。")
        print("\n2. 使用大模型（LLM）从文本中动态生成类型")
        print("   - 适用于特定或未知领域的文本，模型将自动分析并推荐类型。")
        print("\n3. 退出程序")

        choice = input("\n请输入您的选择 (1, 2, 或 3): ").strip()
        if choice in ['1', '2']:
            return int(choice)
        elif choice == '3':
            print("用户选择退出。")
...
def main():
    ...
    # --- 修改：调用新的文件读取函数 ---
    file_contents = read_supported_files(directory_path)
    if not file_contents:
        logging.info("在指定目录未找到任何支持的文本文件（.txt, .pdf, .docx），程序退出。")
        return
    ...
    # --- 6. 修改：Schema 定义工作流 ---
    schema_choice = choose_schema_source()
    final_entities, final_relations = [], []

    if schema_choice == 1:
        # 选项1：使用预定义类型，并允许用户审核修改
        logging.info("使用预定义的Schema。")
        # 将预定义类型交给用户审核，用户可以接受、修改或退出
        final_entities, final_relations = get_user_schema_confirmation(
            PREDEFINED_ENTITY_TYPES,
            PREDEFINED_RELATION_TYPES
        )
    elif schema_choice == 2:
        # 选项2：使用LLM动态生成，并让用户审核修改
        logging.info("使用大模型动态生成Schema。")
    ...
    # --- 修改：实例化并使用 SpacyTextSplitter ---
    logging.info("正在初始化Spacy文本分块器 (使用 'xx_sent_ud_sm' 多语言模型)...")
    # 使用多语言模型 'xx_sent_ud_sm'，它能更好地处理中英文混合文本
    text_splitter = SpacyTextSplitter(pipeline="xx_sent_ud_sm", chunk_size=1000,chunk_overlap=200)
    logging.info("Spacy文本分块器初始化完成。")
    ...
    for file_name, content in file_contents:
        ...
        logging.info(f"文件 {file_name} 通过Spacy分析分成 {len(chunks)} 个块")
